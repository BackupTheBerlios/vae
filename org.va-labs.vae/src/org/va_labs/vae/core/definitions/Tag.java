 ...
 *
 * $Id: Tag.java,v 1.1 2004/08/17 22:38:30 mojo_jojo Exp $
 */
...
import org.va_labs.vae.exceptions.NoSuchTagException;

/**
 * @author mojo_jojo
 * Summaries the methods relative to each tags in the 
 * ant build file.
 * Some of the methods have a base implementation that is supposed to be 
 * redefined by children classes for specific tags.
 * Tags can have an xml name and an "usual" name. 
 * The Xml name is the name of the tag in the build file, whereas the "usual"
 * name is the value of the attribute name or path if they exist.
 */
public abstract class Tag {
	...
	/**
	 * "usual" name of the tag. 
	 * Value of the attribute name or path.
	 */
	...
	/**
	 * Xml name of the project tag.
	 * Name of the xml tag that defined this object.
	 */
	protected StringBuffer tagName;
	
	/**
	 * Indicates wether the project needs to be saved or not.
	 * if false, the project needs to be saved.
	 */
	 ...
	 * All children class should not have to redefine this function: 
	 * special treatments to keep internal tag data in sync with the build
	 * file info should be coded by redefining setKeyAttributes. 
	 ...
	 * @throws IllegalAttributeException if the attribute is not legal for this tag.
	 */
	 ...
	 * @param position position of the attribute in the list of attributes.
	 * @throws IllegalAttributeException if the attribute is not legal for this tag.
	 */
	public void addAttribute(String name, String value, 
			int position)
	 ...
	 * Children class should not have to redefine this function:
	 * Clean status will be set to false during this operation
	 * special treatments to keep internal tag data in sync with the build
	 * file info should be coded by redefining setKeyTag. 
	 * @param tag tag nested inside this one.
	 */
	 ...
	 * Returns true if the tag is unchanged.
	 * @see org.vae.core.Project#getClean()
	 */
	 ...
	 * @param attributeName name of the attribute we are searching.
	 * @param operation operation that requested this search. It is
	 * used if an exception is raised.
	 * @return the position of the attribute.
	 * @throws NoSuchAttributeException if the attribute is not in the 
	 * list.
	 */
	...
	throws NoSuchAttributeException 
	{
		...
		throw new NoSuchAttributeException(operation, attributeName, 
				name.toString());
	}
	
	/**
	 * Locates the position of an a tag in the nested elements list.
	 * @param tag that we are searching. 
	 * @param operation operation that requested this search. It is used
	 * if an exception is raised.
	 * @return the position of the tag.
	 * @throws NoSuchTagException if the tag is not in the list.
	 */
	...
	throws NoSuchTagException
	{
		...
		if (position == -1) {
			throw new NoSuchTagException(tag.getName(), 
					name.toString());
		}
		return position;
	}
	 ...
	 * Clean status will be set to false during this operation
	 * @param position position of the attribute to be removed.
	 * @throws NoSuchAttributeException if there is no attribute at the
	 * specified position.
	 */
	...
	throws NoSuchAttributeException
	{
		try {
			TagAttribute attribute = 
				(TagAttribute) attributes.remove(position);
			removeKeyAttributes(attribute.getName());
			setClean(false);
		} catch(IndexOutOfBoundsException e) {
			throw new NoSuchAttributeException("removal",
					"not specified", name.toString());
		}
	}
	 ...
	 * @throws NoSuchAttributeException in case this attribute doesn't exist.
	 */
	 ...
	 * Name is the only key attribute common to all Tag subclasses.
	 * Subclasses that have other data to sync should redefine this method.
	 * @param attributeName name of the removed attribute.
	 */
	 ...
	 * @param tag tag that is being removed.
	 */
	 ...
	 * Clean status will be set to false during this operation.
	 * @param tag Tag to be removed from the nested tags list
	 * @throws NoSuchTagException if the tag is not in the nested elements
	 * list.
	 * TODO: We need to verify if the call to locateTag is really needed. 
	 * Maybe a simple call to contains and remove could do the trick.
	 */
	 ...
	 * Clean status will be set to false during this operation
	 * @param attributes a list of the different attributes as 
	 * TagAttribute objects.
	 * @throws IllegalAttributeException If one or more of the attributes are not legal
	 * for this tag.
	 */
	...
	/**
	 * Sets wether the project need to be saved or not.
	 * @param status, false means project needs to be saved.
	 * @see org.vae.core.Project#setClean(boolean)
	 */
	 ...
	 * Clean status will be set to false during this operation
	 * @param nestedTags the nested tags for this tag as a list of Tag 
	 * objects.
	 */
	 ...
	 * @param newName the new name of the attribute (may be the same
	 * as originalName).
	 * @param value the new value  of the attribute (may be the same
	 * as the original value).
	 * @throws NoSuchAttributeException if the attribute to be updated does
	 * not exist.
	 * @throws IllegalAttributeException if the attribute is not legal for   
	 * this tag.
	 */
	public void updateAttribute(String originalName, 
			String newName, String value)
	throws NoSuchAttributeException, IllegalAttributeException
	{
		int position = locateAttribute(originalName, "update");
		TagAttribute attrib = 
				(TagAttribute) attributes.get(position);
	 ...
	 * @param value the new value  of the attribute (may be the same
	 * as the original value).
	 * @param position position of the attribute among the other 
	 * attributes.
	 * @throws NoSuchAttributeException if the attribute to be updated does
	 * not exist.
	 * @throws IllegalAttributeException if the attribute is not legal for   
	 ...
	 * appropriate methods.
	 * The implementation at this level is very permissive (everything
	 * goes and only the name gets updated). 
	 * Children ought to redefine this to get a finer control on what 
	 * attributes are legal or not.
	...
	/**
	 * Verify that the nested tag is acceptable for this tag, and sync the
	 * local data with the build data. 
	 * In this implementation it lets everything go through and doesn't
	 * update any data.
	 * It is called from addNestedElement and setNestedElements.
	 * Children that needs to keep data in sync ought to redefine this.
	 * @param tag a nestedTag of this object that has just been inserted in
	 * the data layout.  
	 */
